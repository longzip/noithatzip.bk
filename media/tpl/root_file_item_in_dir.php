<div stt_post="<?php echo $k + 1 ?>" stt="<?php echo $v_attachments['url']; ?>" attachment_id="<?php  ?>"  class="flex-item stt-<?php echo ($k + 1) % 4 ?> box relative<?php if(!empty($active_item)) echo ' active' ?>" style="">
	...
	$timthumb_src = CDN_DOMAIN . '/apps/timthumb/timthumb.php?src=' . SITE_URL . '/' . $v_attachments['url'] . '&w=140&h=140';
	$timthumb_src = $v_attachments['url'];
    ...
    switch( get_file_type($ext) )
    {
        ...
        {
            ?>
            <div real_src="<?php echo SITE_URL, '/', $real_v_attachments['url']; ?>" class="active pointer" src="<?php echo SITE_URL, '/',  $timthumb_src ?>"  >
                <?php 
                    if(empty($uploading))
                    {
                        ?>
                        <img style="width: 100%;"  real_src="<?php echo SITE_URL, '/', $real_v_attachments['url']; ?>" class=" lazyload active pointer" src="<?php echo CDN_DOMAIN ?>/inc/images/loading-lazyloading.gif" data-src="<?php echo SITE_URL, '/',  $timthumb_src ?>" />
                        <?php
                    }
                    else
                    {
                        ?>
                        <img style="width: 100%;"   real_src="<?php echo SITE_URL, '/', $real_v_attachments['url']; ?>" class="   active pointer" src="<?php echo SITE_URL, '/',  $timthumb_src ?>" />
                        <?php
                    }
                ?>
        ...
        {
            ?>
            <div real_src="<?php echo SITE_URL, '/', $real_v_attachments['url']; ?>" class="active pointer box-inner-mp3" src="<?php echo $timthumb_src ?>"  >
                <audio   controls>                               
        ...
        {
            ?>
            <div real_src="<?php echo SITE_URL, '/', $real_v_attachments['url']; ?>" class="active pointer box-inner-video" src="<?php echo $timthumb_src ?>"  >
                <audio   controls>                               
