...
foreach($obj_query->query_terms($moment) as $k_terms=>$v_terms)
{
    ...
    <div class="body-item" particular="<?php echo $v_terms['id'] ?>">
        <div class="stt  fl"><?php echo $k_terms; ?></div>
        <div class="id  fl"><?php echo $v_terms['id'] ?></div>
        <div class="post_title fl"><?php echo $v_terms['title'] ?></div>


        <div class="image text-center fl"><img style="max-height: 50px;max-width:50px;"  src="<?php echo $v_terms['image'] ?>"/></div>
        <div class="action fl">
            <a class="fl" href="<?php echo SITE_URL . '/admin/edit_category/'.$v_terms['id']  ?>">Edit</a>
            <a class="fl" href="<?php echo SITE_URL . '/' . $v_terms['url'] . '-c' . $v_terms['id'] ?>">View</a>
