...
?>
        ...
        <div class="attribute_item">
         ...
         <div class="attribute_item">
            <label>Referer : </label>
            <select class="field_attribute" attribute_name="ref">
                <option <?php if($default['ref'] == '0') echo 'selected' ?> value="0">None</option>
                <?php 
                    $temp_field = get_fields(array('post_type'=>$post_type_id));
                     
                    foreach($temp_field as $v)
                    {
                        $v_attribute = json_decode($v['attribute'], TRUE);
                        //h($v_attribute);
                        ?>
                        <option <?php if($default['ref'] == $v['id']) echo 'selected' ?> value="<?php echo $v['id'] ?>"><?php echo $v_attribute['title'] ?></option>
                        <?php
                    }
                ?>
